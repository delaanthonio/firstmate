...
PATH="$REAL_TASKS_AXI_PATH" exec "$REAL_TASKS_AXI" "$@"
SH
  chmod +x "$home/fakebin/tasks-axi"

  REAL_TASKS_AXI="$real_tasks" REAL_TASKS_AXI_PATH="$TASKS_AXI_BASE_PATH" RECHAIN_FAIL_MARKER="$marker" \
  ...
  out=$(REAL_TASKS_AXI="$real_tasks" REAL_TASKS_AXI_PATH="$TASKS_AXI_BASE_PATH" RECHAIN_FAIL_MARKER="$marker" \
    run_pf "$home" rechain public-final-resume-b --from public-final-resume-a \
      --work-home main --work-id ship-resume --expected pr-merged) \
...
test_pending_skips_concurrent_retirement() {
  ...
  cat > "$home/fakebin/tasks-axi" <<'SH'
#!/usr/bin/env bash
PATH="$REAL_TASKS_AXI_PATH" "$REAL_TASKS_AXI" "$@"
rc=$?
...
SH
  chmod +x "$home/fakebin/tasks-axi"
  REAL_TASKS_AXI="$real_tasks" REAL_TASKS_AXI_PATH="$TASKS_AXI_BASE_PATH" PENDING_LISTED="$home/pending-listed" \
...
PATH="$REAL_TASKS_AXI_PATH" "$REAL_TASKS_AXI" "$@" | jq --argjson invalid "$FAKE_INVALID_KEYS" '
...
SH
  chmod +x "$home/fakebin/tasks-axi"
  REAL_TASKS_AXI="$real_tasks" REAL_TASKS_AXI_PATH="$TASKS_AXI_BASE_PATH" FAKE_INVALID_KEYS=unreadable expect_failure \
  ...
  for invalid in '["pr_url",7]' '["pr_url",""]' '["PR_URL"]' '["pr-url"]'; do
    REAL_TASKS_AXI="$real_tasks" REAL_TASKS_AXI_PATH="$TASKS_AXI_BASE_PATH" FAKE_INVALID_KEYS="$invalid" expect_failure \
