...
pe_register "$HD" lavish argv-src -- "$BLOCKER" "$TRIG3" "one arg with spaces" "second; touch $SHELL_SENTINEL" >/dev/null
...
assert_grep "second; touch $SHELL_SENTINEL" "$R" "a shell-looking argument is passed literally, never interpreted"
assert_absent "$SHELL_SENTINEL" "no shell interpretation occurred"
assert_not_contains "$(wake_payloads "$HD")" "touch $SHELL_SENTINEL" "argv content never reaches the event line"
