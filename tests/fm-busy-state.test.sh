...
test_droid_regex_isolated() {
  local state out
  state=$(new_state_dir droid-arm)
  out=$(fm_busy_classify tmux w1 droid t1 "$state" 'Invoking tools...  (Press ESC to stop)')
  [ "$out" = "busy droid-regex" ] || fail "Droid busy tail must classify 'busy droid-regex', got '$out'"
  out=$(fm_busy_classify tmux w1 droid t1 "$state" 'Ready
> ')
  [ "$out" = "idle droid-regex" ] || fail "Droid idle tail must classify 'idle droid-regex', got '$out'"
  out=$(fm_busy_classify tmux w1 droid t1 "$state" 'Ctrl+c:cancel')
  [ "$out" = "idle droid-regex" ] || fail "a Grok footer must not classify Droid busy, got '$out'"
  pass "the Droid fallback is regex-scoped to Droid and classifies only Droid tasks"
}
