...
test_poll_retries_transient_fetch_and_surfaces_real_metadata() {
  local fakebin homedir_bin real_py real_py_q clean_path_q harness retry_home control out rc=0 wakeq
  fakebin=$(fm_fakebin "$TMP_ROOT")
  ...
  printf -v real_py_q '%q' "$real_py"
  printf -v clean_path_q '%q' "$PATH"
...
#!/bin/bash
PATH=$clean_path_q exec $real_py_q "$harness" "\$@"
EOF
  chmod +x "$fakebin/python3"
...
test_poll_sanitizes_header_fields() {
  local fakebin homedir_bin real_py real_py_q clean_path_q harness
  fakebin=$(fm_fakebin "$TMP_ROOT")
  homedir_bin="$HOME_DIR/bin"
  mkdir -p "$homedir_bin"
  [ -e "$homedir_bin/fm-wake-lib.sh" ] || ln -s "$ROOT/bin/fm-wake-lib.sh" "$homedir_bin/fm-wake-lib.sh"
