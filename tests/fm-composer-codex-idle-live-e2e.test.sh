...
# Disable only the startup update check. Its modal is not composer furniture,
# and dismissing it with Escape exits codex 0.154.0 after drawing the idle
# screen into scrollback, which would leave this guard classifying a dead pane
# instead of the installed harness's live composer.
...
while [ "$i" -lt "$budget" ]; do
  ...
  captured=$(tmux capture-pane -e -p -t "$SESSION:$WIN" 2>/dev/null)
  # Herdr's recent read and Zellij's screen dump stop at their rendered
  # content, while tmux pads a tall viewport with rows of spaces below a
  # top-anchored Codex composer. Trim only that blank viewport suffix before
  # taking the same bounded styled tail, or this proxy would classify eighteen
  # padding rows instead of the screen those cursorless backends return.
  last_rendered=$(printf '%s\n' "$captured" | fm_composer_strip_ansi |
    awk '/[^[:space:]]/ { last = NR } END { print last + 0 }')
  if [ "$last_rendered" -gt 0 ]; then
    styled=$(printf '%s\n' "$captured" |
      sed -n "1,${last_rendered}p" | tail -n "$FM_COMPOSER_CAPTURE_LINES")
  else
    styled=''
  fi
