...
tmux_new_session() {
  env SHELL=/bin/bash tmux -L "$TMUX_SOCKET" -f /dev/null new-session "$@"
}
...
test_operational_followup_turn_e2e() {
  local project home config sessions version label case_name calm_state expected_notifications session_file pane i captain_line handled_line geometry_gap exact_session ready_diag
...
TS
  ...
  run_followup_case() {
    ...
    tmux_new_session -d -s "$TMUX_SESSION" -x 160 -y 36 \
      "cd '$project' && env FM_HOME='$home' PI_CODING_AGENT_DIR='$config' FM_OPERATIONAL_INPUT_SCRIPT='$OPERATIONAL_INPUT' PI_OFFLINE=1 pi --approve --no-context-files --no-skills --no-prompt-templates --no-extensions $extensions --model followup-e2e/deterministic $session_arg; rc=\$?; printf '\nPI_EXIT=%s\n' \"\$rc\"; sleep 20"
    i=0
    while [ "$i" -lt 120 ]; do
    ...
    if ! printf '%s\n' "$pane" | grep -Fq 'followup-e2e.ts'; then
      ready_diag=$(printf '%s\n' "$pane" | tail -20 | tr '\n' '|')
      fail "Pi follow-up $case_name case ($label) did not reach the ready composer; pane tail: $ready_diag"
    fi
...
JS
  ...
  replay_exact_case() {
    ...
    printf '%s\n' on >"$home/config/calm"
    tmux_new_session -d -s "$TMUX_SESSION" -x 160 -y 36 \
      "cd '$project' && env FM_HOME='$home' PI_CODING_AGENT_DIR='$config' FM_OPERATIONAL_INPUT_SCRIPT='$OPERATIONAL_INPUT' PI_OFFLINE=1 pi --approve --no-context-files --no-skills --no-prompt-templates --no-extensions -e ./.pi/extensions/fm-calm.ts -e ./followup-e2e.ts --model followup-e2e/deterministic --session '$exact_session'; rc=\$?; printf '\nPI_EXIT=%s\n' \"\$rc\"; sleep 20"
    i=0
    while [ "$i" -lt 120 ]; do
      pane=$(tmux -L "$TMUX_SOCKET" capture-pane -p -t "$TMUX_SESSION" -S - 2>/dev/null || true)
...
TS
  ...
  start_geometry_pi() {
    local session_arg=$1
    tmux -L "$TMUX_SOCKET" kill-session -t "$TMUX_SESSION" 2>/dev/null || true
    tmux_new_session -d -s "$TMUX_SESSION" -x 100 -y 44 \
...
JSON
  ...
  tmux_new_session -d -s "$TMUX_SESSION" -x 180 -y 44 \
