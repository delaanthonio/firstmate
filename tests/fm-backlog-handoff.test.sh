...
TASKS_AXI_REAL=$(command -v tasks-axi)
...
EOF
  printf '## Queued\n\n## Done\n' > "$sub/data/backlog.md"
  real_tasks=$TASKS_AXI_REAL
  fm_fake_crash_injector "$fakebin"
  cat > "$fakebin/tasks-axi" <<'SH'
#!/usr/bin/env bash
PATH="$FM_REAL_TASKS_AXI_PATH" "$FM_REAL_TASKS_AXI" "$@"
rc=$?
case " $* " in
  *" --file "*" --to "*)
...
SH
  ...
  set +e
  FM_REAL_TASKS_AXI="$real_tasks" FM_REAL_TASKS_AXI_PATH="$TASKS_AXI_BASE_PATH" \
...
PATH="$FM_REAL_TASKS_AXI_PATH" exec "$FM_REAL_TASKS_AXI" "$@"
SH
  chmod +x "$fakebin/tasks-axi"
  : > "$TMP_ROOT/default-tmux.log"
