...
test_secondmate_home_teardown_delivers_final_line_or_refuses() {
  local case_dir rc channel wt_head err seq generation task_tmp tag
  ...
  channel="$case_dir/parent/state/mate-x.status"
  write_meta "$case_dir" local-only ship
  tag=$(FM_HOME="$case_dir/home" FM_ROOT="$case_dir/home" \
    bash -c '. "$1"; fm_backend_hometag' _ "$ROOT/bin/fm-backend-hometag-lib.sh")
  ...
  [ ! -e "$task_tmp" ] || fail "mate-teardown-refuses: rerun left the task temp root"
