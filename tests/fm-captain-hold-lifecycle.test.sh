...
TASKS_AXI_SHIM=$TASKS_AXI_BIN
printf -v tasks_axi_shim_q '%q' "$TASKS_AXI_SHIM"
printf -v tasks_axi_path_q '%q' "$PATH"
TASKS_AXI_BIN="$TMP_ROOT/tasks-axi-real"
cat > "$TASKS_AXI_BIN" <<SH
#!/usr/bin/env bash
PATH=$tasks_axi_path_q exec $tasks_axi_shim_q "\$@"
SH
chmod +x "$TASKS_AXI_BIN"
