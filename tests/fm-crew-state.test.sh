...
test_same_run_done_transitions_have_distinct_events() {
  ...
  assert_contains "$checks_out" "event: run:01RUN:outcome:checks-passed" \
    "checks-passed exposes its exact done transition"
  FM_FAKE_AXI_STATUS="$(run_passed fm/feat-done-events)"
  passed_out=$(run_crew_state "$d" done-events)
  assert_contains "$passed_out" "event: run:01RUN:outcome:passed" \
    "passed exposes its exact done transition"
  [ "$checks_out" != "$passed_out" ] \
    || fail "distinct done transitions emitted the same authoritative state receipt"
...
test_equal_second_terminal_failed_then_declared_pause() {
  ...
  [ "$(mtime_seconds "$d/state/feat-failed-pause.status")" \
      = "$(sqlite3 "$d/nmhome/state.sqlite" 'SELECT completed_at FROM step_results;')" ] \
    || fail "failure-then-pause regression did not model equal-second events"
  FM_FAKE_AXI_STATUS="$(run_failed fm/feat-failed-pause)"
  out=$(run_crew_state "$d" feat-failed-pause)
...
test_equal_second_declared_pause_then_terminal_failure() {
  reset_fakes
  local d out
  d=$(new_case paused-then-failed)
  make_repo_on_branch "$d/wt" fm/feat-pause-failed
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-pause-failed.meta" \
    "window=fm:fm-feat-pause-failed" "worktree=$d/wt" "kind=ship"
...
test_unmarked_pause_after_terminal_failure() {
  reset_fakes
  local d out
  d=$(new_case unmarked-failed-then-paused)
  make_repo_on_branch "$d/wt" fm/feat-unmarked-failed-pause
  make_fakebin "$d" >/dev/null
  make_linux_stat_fakes "$d"
  fm_write_meta "$d/state/unmarked-failed-pause.meta" \
    "window=fm:fm-unmarked-failed-pause" "worktree=$d/wt" "kind=ship"
  ...
  sqlite3 "$d/nmhome/state.sqlite" \
    "UPDATE runs SET terminal_head_verified_at = NULL WHERE id = '01RUN';"
  set_mtime "$d/state/unmarked-failed-pause.status" 1700000010
  FM_FAKE_AXI_STATUS="$(run_failed fm/feat-unmarked-failed-pause)"
  out=$(run_crew_state "$d" unmarked-failed-pause)
...
test_unmarked_pause_before_terminal_failure() {
  reset_fakes
  local d out
  d=$(new_case unmarked-paused-then-failed)
  make_repo_on_branch "$d/wt" fm/feat-unmarked-pause-failed
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/unmarked-pause-failed.meta" \
    "window=fm:fm-unmarked-pause-failed" "worktree=$d/wt" "kind=ship"
  ...
  out=$(run_crew_state "$d" unmarked-pause-failed)
...
test_unmarked_pause_ambiguous_same_second_surfaces_failure() {
  reset_fakes
  local d out
  d=$(new_case unmarked-ambiguous-order)
  make_repo_on_branch "$d/wt" fm/feat-unmarked-ambiguous
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/unmarked-ambiguous.meta" \
    "window=fm:fm-unmarked-ambiguous" "worktree=$d/wt" "kind=ship"
  ...
  out=$(run_crew_state "$d" unmarked-ambiguous)
  assert_contains "$out" "state: failed" \
    "an ambiguous same-second unmarked pause must surface the failure"
  assert_contains "$out" "source: run-step" \
    "ambiguous legacy ordering remains terminal-authoritative"
