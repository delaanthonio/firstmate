...
if [ "$PRIMARY_WAVE_STATUS" -ne 0 ]; then
  grep -F "presentation recovery could not acquire its session lock; refusing a concurrent resume" \
  ...
  spawn_task "$PRIMARY_WAVE_ID" "$HOME_DIR" "$RECOVERY_PROJECT_DIR" \
    > "$TMP_ROOT/primary-wave-resume-retry.out" 2> "$TMP_ROOT/primary-wave-resume-retry.err" \
    || fail "concurrent primary recovery retry failed: $(cat "$TMP_ROOT/primary-wave-resume-retry.err")"
fi
if [ "$BRAVO_WAVE_STATUS" -ne 0 ]; then
  ...
  spawn_task "$BRAVO_WAVE_ID" "$SECOND_HOME_B" "$RECOVERY_PROJECT_DIR" \
    > "$TMP_ROOT/bravo-wave-resume-retry.out" 2> "$TMP_ROOT/bravo-wave-resume-retry.err" \
    || fail "concurrent secondmate recovery retry failed: $(cat "$TMP_ROOT/bravo-wave-resume-retry.err")"
fi
...
pass "real Herdr lab: concurrent cross-home recoveries serialize or retry the exact bounded-lock refusal, then replace exact husks with no focus drift"
