...
# Last: the foreground process is a plain `sleep`, so the pane never proves an
# empty composer. Depending on the Herdr capture version, the leftover viewport
# is either unknown or visibly pending. Both classifications must refuse before
# typing the exit command into a live agent that ignores it.
start_agent_process
herdr pane report-agent "$PANE_ID" --source fm-control-smoke --agent fm-control-smoke-agent \
  --state idle --session "$SESSION" >/dev/null 2>&1 \
...
case "$OUT" in
  ...
  *) fail "the exit failure should identify an unknown or visibly pending composer, got: $OUT" ;;
esac
pass "real herdr: an agent behind an unknown or visibly pending composer is refused before any exit command is typed"
