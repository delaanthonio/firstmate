...
if [ "${FM_HERDR_STATEFUL_SERVER:-0}" = 1 ]; then
  if [ "${1:-}" = server ]; then
    : > "$RESP/.server-running"
    exit 0
  fi
  if [ "${1:-}" = status ] && [ "${2:-}" = --json ]; then
    running=false
    [ -f "$RESP/.server-running" ] && running=true
...
test_container_ensure_starts_server_and_workspace() {
  ...
  # Status reads are backed by fake server state so the backgrounded `herdr
  # server` process and the readiness poll cannot race over numbered replies.
  # 1: workspace list -> empty (no "firstmate" workspace yet)
  printf '{"result":{"workspaces":[]}}\n' > "$resp/1.out"
  # 2: workspace create -> w1, seeding default tab w1:t9 (real herdr returns
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" FM_HERDR_SCRIPT_STATUS=1 FM_HERDR_STATEFUL_SERVER=1 HERDR_SESSION=fmtest \
...
test_composer_state_bare_prompt_is_empty() {
  ...
  [ "$out" = empty ] || fail "a bare prompt glyph should read as empty in the C locale, got '$out'"
  pass "fm_backend_herdr_composer_state: a bare '❯' composer row reads empty in the C locale"
}
...
test_composer_state_ghost_placeholder_is_empty() {
  ...
  out=$( LC_ALL=C PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_composer_state default:w1:p2' "$ROOT" )
  [ "$out" = empty ] || fail "the known ghost placeholder 'Type a message...' should read as empty in the C locale, got '$out'"
  pass "fm_backend_herdr_composer_state: the ghost placeholder text reads empty in the C locale, not pending"
}
