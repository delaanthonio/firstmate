...
| `fm-brief.sh`            | Scaffold ship (explicit `--mode`), scout, secondmate-charter, and Herdr-lab briefs, rendering shared delivery contracts for ship tasks and Captain's intent and Firstmate spec subsections on ship/scout |
| [`fm-dod-lib.sh`](../bin/fm-dod-lib.sh) | Own ship/scout worker role scope, ship review-evidence and definitions-of-done blocks, and the no-mistakes `--intent` contract |
...
| `fm-turnend-guard.sh`    | Shared primary turn-end guard for watcher liveness and authenticated due standing checks (docs/turnend-guard.md) |
...
| `fm-droid-afk-askuser-check.sh` | Droid-only PreToolUse guard that files and defers interactive approval questions while away mode is active |
...
| `fm-supervision-lib.sh`  | Shared watcher-health predicates and authenticated serialized turn-end check backstop |
...
| `fm-promote.sh`          | Promote a scout task in place to a protected ship task with an explicit delivery mode, write the ship instructions carrying that mode's shared delivery contract, and supersede the task's brief so a later relaunch cannot revive stale scout delivery text |
