...
| `fm-brief.sh`            | Scaffold ship briefs with a code-quality pass and mode-specific done contract, report-only scout briefs, secondmate charters, and Herdr-lab briefs |
...
| `fm-spawn.sh`            | Spawn crewmates, scouts, `id=repo` batches, and secondmates on the resolved harness and runtime backend; tmux targets its resolved session explicitly |
