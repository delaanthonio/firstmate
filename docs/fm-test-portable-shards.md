...
Thirty-one scripts added after those CI runs carry provisional hints from two complete local green measurements on 2026-09-20, retaining the slower green value where both runs covered the script.
Those local values prevent the serial coverage guard from balancing a large new cohort entirely on its default guess, but they are not CI-equivalent evidence and must be replaced with maxima from the first several complete green portable-serial CI artifacts that include those scripts.
