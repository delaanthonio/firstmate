...
The daemon-backed regression separately exercises launcher crash and restart end to end for tmux in an isolated private socket, including preservation of already-staged delivery across same-lifecycle daemon replacement.
Its direct and native lifecycle cases are behaviorally mocked rather than live backend runs.
Herdr and Droid daemon recovery was characterized live before that correction, but corrected same-lifecycle crash recovery has not been rerun live on either backend.
...
The compatibility floor is Zellij 0.44.0, and the latest verification used Zellij 0.45.1 with `jq` on macOS aarch64 on 2026-09-20.
