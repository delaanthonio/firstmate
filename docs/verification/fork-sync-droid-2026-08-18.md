...
The resulting fork-specific delta was inspected with `git diff upstream/main..HEAD`.
...
- Droid ship and scout launches retain `droid --settings <state-file> --auto high <brief>`.
- Droid secondmate launches retain the verified base `droid --auto high <brief>` shape.
- The per-task `state/<id>.droid-settings.json` Stop hook remains generated by `bin/fm-spawn.sh`.
...
- Operator guidance remains in `AGENTS.md`, `README.md`, `docs/configuration.md`, `docs/agent-control.md`, `.agents/skills/afk/SKILL.md`, and `.agents/skills/harness-adapters/SKILL.md`.
