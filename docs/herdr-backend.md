...
The adapter is a thin capture: it hands a bounded ANSI tail plus Herdr's capability facts to the fleet-wide classifier in `bin/fm-composer-lib.sh`, which owns every shape - bordered boxes, bare agent-glyph rows (including muse's `⟩`, which the adapter's retired local pattern silently omitted), opencode's left bar, and identity-corroborated ambiguous rows.
The classifier admits Pi's separator region only when native `agent get` identity is exactly Pi and state is idle or done, and admits Claude's fully de-emphasized `❯ <suggestion>` row only when that identity is exactly Claude and state is idle or done.
...
A blocked or working Claude, normal-intensity draft text, missing or mismatched identity, a working Pi, pending middle row, incomplete separator pair, or over-tall candidate remains unknown or pending.
Identity stays a lazy second read, consulted only when one of those identity-dependent shapes could change the verdict.
...
The durable `state/.afk` flag identifies the away lifecycle across daemon and primary-session restarts rather than relying on a transient daemon PID.
Recovery with that flag present preserves staged escalation delivery, while a genuinely fresh entry with no flag clears prior-lifecycle artifacts.
Durable queue and task records remain authoritative source state, and interruption before acknowledgement can replay a source row rather than promising general exactly-once delivery.
