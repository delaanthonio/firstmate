...
Ship tasks change projects and ship by project mode (`no-mistakes`, `direct-PR`, or `local-only`), with review-evidence contracts owned by `bin/fm-dod-lib.sh` and rendered by `bin/fm-brief.sh` or `bin/fm-promote.sh`; scout tasks leave standalone investigation reports at `data/<id>/report.md` and never push.
