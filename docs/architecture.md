...
A causally later declared pause can supersede a failed or cancelled no-mistakes run.
The `fm-crew-state.sh` header owns the exact run binding and legacy ordering rules.
...
A matching terminal run normally remains authoritative, but a causally later declared pause may supersede a failed or cancelled run.
The `fm-crew-state.sh` header owns the exact run-head ancestry, pause syntax, ordering, and terminal-event identity rules.
...
Ship tasks change projects and ship by project mode (`no-mistakes`, `direct-PR`, or `local-only`) with review-ready descriptions and user-visible UI evidence; scout tasks leave standalone investigation reports at `data/<id>/report.md` and never push.
...
That keeps spawn launch compatible across claude, codex, opencode, pi, pi-signed, grok, kimi, cursor, droid, gemini, muse, rovo, omp, and agy while preserving the requested profile for later audit.
...
`bin/fm-brief.sh` owns the detailed ship-only review-evidence contract: PR-producing modes require a four-section plain-language description and attach UI evidence, local-only keeps that evidence in task data, and non-UI work explicitly marks screenshots not applicable.
Scout and secondmate briefs omit that ship-only contract.
