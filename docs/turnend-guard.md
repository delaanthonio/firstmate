...
## Standing Check Backstop

The watcher remains the normal owner of `state/*.check.sh`, but the turn-end guard runs the same due-check path after primary scoping and before watcher-liveness evaluation.
This closes the gap where a merge, CI, or no-mistakes poll becomes actionable while the primary is still in a turn and the watcher has not yet surfaced it.
The same behavior applies inside a genuine secondmate home because that home runs its own primary firstmate session, while child crewmate and scout worktrees remain out of scope.

`bin/fm-supervision-lib.sh` owns the shared `fm_supervision_run_due_checks <state-dir> <interval-seconds> <timeout-seconds> [log-errors]` implementation.
Both `bin/fm-watch.sh` and `bin/fm-turnend-guard.sh` call it.
The runner skips when no check exists or the shared `.last-check` cadence is fresh, acquires `.last-check.lock`, and rechecks the cadence under that lock so watcher and turn-end paths cannot double-run a sweep.
Each script runs under `FM_CHECK_TIMEOUT`, using `timeout`, `gtimeout`, or a process-group-killing Perl fallback.
The first actionable stdout result is appended durably to `.wake-queue` through `fm_wake_append` before `.last-check` is stamped.
Silent, failing, and timed-out sweeps also stamp `.last-check`, preventing one broken check from wedging every turn end.
Check failures and timeouts fail open; the turn-end guard logs concise context while the watcher preserves its quiet behavior.

When a due check prints actionable output, the guard queues the wake and emits a bordered `TURN WOULD END WITH A DUE CHECK WAKE` reason before blocking once.
The existing `stop_hook_active` and passive-adapter loop guards still bound that intervention to one continuation per logical turn.
...
`tests/fm-standing-checks.test.sh` covers due cadence, durable queueing, locking, timeout bounds, and fail-open check behavior.
`tests/fm-turnend-guard.test.sh` covers the shared predicate, due-check blocking, primary scoping (including a secondmate's own home being guarded like the main primary while its child worktrees stay exempt), `FM_HOME` and `FM_STATE_OVERRIDE` precedence, Pi logical-run latch behavior for no-tool and multi-tool runs, fail-open behavior without `jq`, tracked hook registration for all five harnesses, and the Grok adapter's forced-resume loop guard and permission-mode regression.
