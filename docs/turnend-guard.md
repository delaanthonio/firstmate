...
The Relay shim, registered PR polls, and trusted custom checks retain the watcher's validation and immutable-snapshot rules, and each execution remains bounded by `FM_CHECK_TIMEOUT`.
An unauthenticated state check is never executed and its rejection becomes an actionable notification; the due sweep scans every matching check before publication, so a rejection after valid actionable output is included in that same durable notification rather than hidden.
An execution failure or timeout is logged and lets the turn end continue without that optional protection.
...
- Droid registers a `Stop` hook in `.factory/settings.json`, anchors through `DROID_PROJECT_DIR`, and passes the native payload to the shared guard.
...
Claude, Codex, and Droid can block a Stop directly with exit status 2 and stderr.
All three payloads carry `stop_hook_active`.
In the default Codex and Droid mode, a true value lets the second stop finish after one forced continuation.
...
- Droid primary hooks load from tracked project-level `.factory/settings.json` on a normal interactive `droid` invocation; the headless surface is unverified.
...
`tests/fm-standing-checks.test.sh` covers authenticated due-check cadence, durable queueing, locking, timeout bounds, and continuation behavior when an optional check cannot complete.
...
[`verification/droid-primary.md`](verification/droid-primary.md#stop-blocking-and-loop-guard) records Droid's blocking Stop and loop-guard evidence.
