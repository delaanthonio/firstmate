...
FM_OPENCODE_ARM_READY_TIMEOUT_MS=16000   # minimum milliseconds the OpenCode primary watcher plugin waits for an arm result; effective wait also covers selected arm confirmation seconds plus one-second rounding and five-second grace; default minimum 36000 on Windows
FM_PI_ARM_READY_TIMEOUT_MS=16000   # minimum milliseconds the Pi watcher extension waits for a successor arm result; effective wait also covers selected arm confirmation seconds plus one-second rounding and five-second grace; default minimum 36000 on Windows
