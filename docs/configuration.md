...
claude, codex, opencode, pi, pi-signed, grok, kimi, cursor, and droid are empirically verified for crewmate and secondmate launches; [README requirements](../README.md#requirements) own the set supported for the primary session.
...
Claude's Stop `asyncRewake` hook owns tokenless re-arm cycles, Cursor's stop hook parks on the watcher, Grok uses background-notify cycles, Codex and Droid use bounded foreground checkpoints, Pi and pi-signed use the same two tracked primary extensions, and OpenCode uses its TUI plugin.
...
For every template-backed Droid launch, `fm-spawn.sh` writes `state/<id>.droid-settings.json` atomically and passes it with `--settings`.
The process-only file carries requested model and effort overrides, plus the Stop notification hook for crews and scouts; secondmate files omit the hook.
The raw-command escape hatch bypasses adapter settings generation.
Every template-backed Droid spawn requires `jq` before backend allocation, and teardown removes the published settings file with the task.
...
FM_DROID_WATCH_CHECKPOINT=180   # seconds per foreground watcher checkpoint in Droid primary supervision
FM_CREW_STATE_NM_TIMEOUT=10   # seconds allowed per no-mistakes query inside fm-crew-state.sh
FM_TEARDOWN_NM_TIMEOUT=10    # seconds allowed per no-mistakes query or abort inside fm-teardown.sh
FM_CREW_STATE_RUNS_LIMIT=200  # recent no-mistakes run rows scanned when axi status cannot be attributed to the current code
FM_CREW_STATE_BIN=bin/fm-crew-state.sh   # test override for the current-state reader used by watcher working, pause, and terminal-transition triage
...
FM_OPENCODE_ARM_READY_TIMEOUT_MS=16000   # minimum milliseconds the OpenCode primary watcher plugin waits before and after the arm publishes its confirmation boundary; the post-boundary wait also covers selected arm confirmation seconds plus one-second rounding and five-second grace; default minimum 36000 on Windows
FM_PI_ARM_READY_TIMEOUT_MS=16000   # minimum milliseconds the Pi watcher extension waits before and after a successor arm publishes its confirmation boundary; the post-boundary wait also covers selected arm confirmation seconds plus one-second rounding and five-second grace; default minimum 36000 on Windows
FM_WATCH_ARM_RETIRE_TIMEOUT_MS=4000   # milliseconds Pi/OpenCode wait for an unready successor arm to exit before abandoning retries
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; other stale panes surface immediately unless current-state reconciliation confirms a declared pause
...
FM_BUSY_REGEX=          # optional override for rendered delivery guards and the isolated Grok and Droid task-state fallbacks; converted worker state ignores it
