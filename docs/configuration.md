...
Zellij has no per-home workspace split: primary and secondmate tasks share that one session, callers keep using `fm-<id>`, and visible tab titles are scoped by the active `FM_HOME` readable label plus a short hash of the resolved `FM_HOME` path as `fm-<home-label>-<id>`.
...
claude, codex, opencode, pi, droid, and grok are all empirically verified; new harnesses get verified through a supervised trial task before joining the set.
...
FM_BUSY_REGEX='esc (to )?interrupt|Working\.\.\.|Press ESC to stop|Ctrl\+c:cancel'   # busy-pane signatures, shared by watcher, fm-crew-state pane fallback, and tmux helper
