...
| Droid | Run | `.factory/settings.json` registers `SessionStart`, anchored through `$DROID_PROJECT_DIR`, and invokes `bin/fm-sessionstart-run.sh`. | The payload supplies `source: startup`, and hook stdout reaches model context on a normal interactive invocation. |
