...
Before /no-mistakes, the no-mistakes ship brief has the crewmate run a CodeRabbit pre-pass on its own changes: `coderabbit review` (or `--agent` for structured findings), fix the real findings, dismiss false positives in the reply.
This precedes, and never replaces, the no-mistakes review gate - it just hands the gate already-cleaner code.
If `coderabbit doctor` reports CodeRabbit is unauthenticated, the crewmate surfaces it as a `needs-decision` rather than silently skipping: it is a captain setup step like `gh auth login`, resolved with `coderabbit auth login`, which only the captain can run.
...
The no-mistakes brief points to no-mistakes' version-matched guidance and keeps only firstmate-specific wrapper rules for the CodeRabbit pre-pass (run `coderabbit review` and fix real findings before /no-mistakes; surface an unauthenticated CodeRabbit as a captain setup step, never silently skip), `ask-user` escalation, `--yes` avoidance, and the CI-green done line.
