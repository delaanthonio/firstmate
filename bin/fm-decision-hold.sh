...
#   preserve-question | open-questions     -> unchanged, owned by captain-hold
set -eu
...
case "${1:-}" in
  ...
  preserve-question) shift; exec "$CAPTAIN_HOLD" preserve-question "$@" ;;
  open-questions) shift; exec "$CAPTAIN_HOLD" open-questions "$@" ;;
  -h|--help) usage ;;
  *) usage >&2; exit 2 ;;
esac
