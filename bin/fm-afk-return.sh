...
#
# When Droid deferred an AskUser call, catch-up also presents every exact
# outstanding question from the captain-call owner. Presentation never answers
# or approves the question.
set -u
...
print_open_questions() {
  local rc=0
  "$SCRIPT_DIR/fm-captain-hold.sh" open-questions --render || rc=$?
  if [ "$rc" -ne 0 ]; then
    printf 'outstanding decisions limitation: the outstanding-question listing could not be projected; run bin/fm-captain-hold.sh open-questions --render before ordinary work\n'
    return "$rc"
  fi
}
...
return_reconcile() {
  local evidence blockers drain_err questions drained wake_ack_line wake_ack_through wake_ack_generation wedge escalations lifecycle_ok=1 since contract_since superseded_record retained_record
  ...
  questions=$(mktemp "$STATE/.afk-return-questions.XXXXXX") \
...
EOF
  ...
  if [ "$lifecycle_ok" -ne 1 ] || grep -q "^blocker$(printf '\t')" "$blockers"; then
    write_gate "$evidence" "$blockers" \
      || { rm -f "$evidence" "$blockers" "$drain_err" "$questions"; return 1; }
