...
preflight_task_backend_ownership() {
  ...
  if [ "$backend" = zellij ]; then
    (
      ...
      FM_HOME=$home FM_ROOT=$root FM_STATE_OVERRIDE=$state \
        fm_backend_zellij_endpoint_ownership_preflight "$target" "$tab_id" "fm-$task_id"
    ) || return 1
  elif [ "$backend" = cmux ]; then
    (
      unset FM_ROOT_OVERRIDE
      FM_HOME=$home FM_ROOT=$root FM_STATE_OVERRIDE=$state \
        fm_backend_cmux_endpoint_ownership_preflight "$target" "fm-$task_id"
    ) || return 1
  fi
}
...
cleanup_firstmate_child_backend_endpoint() {
  ...
  if [ "$backend" = zellij ]; then
    (
      unset FM_ROOT_OVERRIDE
      FM_HOME=$home FM_ROOT=$root FM_STATE_OVERRIDE=$sub_state \
...
cleanup_firstmate_home_children() {
  local home=$1 root=$2 sub_state child_meta child_id child_t child_wt child_proj child_kind child_home child_backend child_orca_worktree_id child_return_rc child_busy_gen child_owner_rc child_tmp child_zellij_session child_zellij_fingerprint child_zellij_sidecar_fingerprint
  sub_state="$home/state"
  [ -d "$sub_state" ] || return 0
  for child_meta in "$sub_state"/*.meta; do
    [ -e "$child_meta" ] || continue
    ...
    if [ -n "$child_t" ]; then
      if [ "$child_backend" = herdr ]; then
        ...
        if ! cleanup_firstmate_child_backend_endpoint "$home" "$root" "$sub_state" \
            "$child_backend" "$child_t" "$(meta_value "$child_meta" zellij_tab_id)" "$child_id"; then
...
if [ "$KIND" = secondmate ]; then
  ...
  if [ "$FORCE" = "--force" ]; then
    ...
    preflight_firstmate_home_backend_ownership "$HOME_PATH" "$FM_ROOT" || {
...
elif [ "$KIND" != secondmate ] && ! task_tmp_is_legacy_preserve_only "$TASK_TMP" "$ID"; then
...
rm -f "$STATE/$ID.turn-ended" "$STATE/$ID.progress" \
  ...
  "$STATE/.$ID.branch-outcome-index" "$STATE/$ID.zellij-session-fingerprint"
