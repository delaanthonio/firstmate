...
#                          including across pane-hash churn. A newer terminal run
#                          transition surfaces, and its exact event identity
#                          suppresses only a replay of that same transition,
...
# remaining rendered-text fallbacks (Grok and Droid only).
...
# consumed only by the Grok- or Droid-scoped fallback inside the contract.
...
handle_paused_stale() {  # <window> <task> <hash> [terminal]
  local win=$1 task=$2 h=$3 mode=${4:-paused} key statusf mtime age detail reason declaration last until now min_age
  key=$(window_key "$win")
  ...
  if [ "$mode" = terminal ]; then
    triage_log "absorbed stale (authoritative terminal transition already surfaced): $win"
    return 0
  fi
...
pause_state_class() {  # <window> <task>
  ...
  # Re-read authoritative state even for an already-tracked pause. A terminal
  # run transition is newer evidence than the older status declaration and must
  # surface once rather than inheriting the pause cadence.
  kind=$(window_kind "$win")
  class=$(crew_state_class "$task")
  case "$class" in
    working|terminal:*)
      [ "$class" = working ] && rm -f "$recheck_file"
      printf '%s' "$class"
      return
      ;;
  esac
...
surface_nonterminal_stale() {  # <window> <hash> [terminal-receipt]
  local win=$1 h=$2 receipt=${3:-} key task last declared=1 bounded=1 throttled=1 until now
  key=$(window_key "$win")
  ...
  if [ "$declared" -eq 0 ]; then
    : > "$STATE/.paused-$key"
    if [ -n "$receipt" ]; then
      printf '%s' "$receipt" > "$STATE/.paused-rechecked-$key"
    else
    ...
    if [ "$h" = "$prev" ]; then
      ...
      if [ "$n" -ge 2 ] && [ "$busy_now" -ne 0 ]; then
        ...
        if [ "$kind" = secondmate ]; then
          ...
          # The first sight of each stale hash and every already-tracked pause use
          # pause_state_class to reconcile current state. It returns:
          ...
          #   - terminal:<state>:<event>: an authoritative terminal transition,
          #     which surfaces once per exact identity even behind an older pause;
