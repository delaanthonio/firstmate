...
fm_dod_block() {  # <mode> <task-id>
  local mode=$1 id=$2
  local quality_standard="Before reporting done, make the change beautiful: match surrounding style and naming, remove dead code, choose the right abstraction level, keep comments and docstrings evergreen, and run the project's formatter."
  case "$mode" in
    direct-PR)
      cat <<EOF
# Definition of done
...
When it is implemented and committed, append \`done: ready in branch fm/$id\` with any evidence suffix required by the task instructions to the status file and stop.
