...
# Claude, Codex, and Droid can block directly by preserving exit status 2 and stderr.
...
#
# Loop-guard, Codex/Droid/Grok (default) mode: never block twice in the same turn.
# Codex and Droid use stop_hook_active, while Grok uses stopHookActive; typed camel-case
