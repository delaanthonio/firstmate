...
export FM_STATE_OVERRIDE=$(printf '%q' "${FM_STATE_OVERRIDE:-}")
export FM_CONFIG_OVERRIDE=$(printf '%q' "${FM_CONFIG_OVERRIDE:-}")
...
crew_dispatch_validate() {
  ...
  err=$(jq -r '
    def verified($h): ["claude","codex","opencode","pi","droid","grok"] | index($h);
    def effort_ok($h; $e):
      if $e == null then true
      elif ($e | type) != "string" then false
      elif $h == "claude" then (["low","medium","high","xhigh","max"] | index($e))
      elif ($h == "codex" or $h == "grok" or $h == "pi") then (["low","medium","high","xhigh"] | index($e))
      elif ($h == "opencode" or $h == "droid") then false
      else true
      end;
    def bad_efforts:
