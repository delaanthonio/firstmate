...
#   /updatefirstmate, restart). A bare adapter name (claude|codex|opencode|pi|pi-signed|grok|kimi|cursor|muse|droid)
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  case "${POS[1]:-}" in
    ''|claude|codex|opencode|pi|pi-signed|grok|kimi|cursor|muse|droid)
      ARG3=${POS[1]:-}
      ;;
    *' '*)
...
case "$HARNESS" in
  claude|codex|opencode|pi|pi-signed|grok|kimi|muse|droid)
