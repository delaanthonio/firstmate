...
#   /updatefirstmate, restart). A bare adapter name (claude|codex|opencode|pi|pi-signed|grok|kimi|cursor|droid|gemini|muse|rovo|omp|agy)
...
spawn_remote_secondmate() {
  ...
  case "$harness" in
  claude | codex | opencode | pi | pi-signed | grok | kimi | cursor | droid) ;;
  *)
    fm_lock_release "$registry_lock" || true
    fm_lock_release "$SPAWN_TASK_LOCK" || true
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  case "${POS[1]:-}" in
  '' | claude | codex | opencode | pi | pi-signed | grok | kimi | cursor | droid | gemini | muse | rovo | omp | agy)
    ARG3=${POS[1]:-}
    ;;
  *' '*)
...
if [ "$KIND" = secondmate ] && [ -z "$ARG3" ]; then
  ...
  if [ "$EFFORT_SET" -eq 0 ]; then
    ...
    if [ -n "$SM_EFFORT" ]; then
      case "$SM_EFFORT" in
      low | medium | high | xhigh | max | ultra | dynamic) EFFORT=$SM_EFFORT ;;
      *) echo "warning: config/secondmate-harness effort token '$SM_EFFORT' is not one of low, medium, high, xhigh, max, ultra, dynamic; ignoring" >&2 ;;
      esac
    fi
  fi
fi
...
case "$HARNESS" in
claude | codex | opencode | pi | pi-signed | grok | kimi | droid | gemini | muse | rovo | agy)
