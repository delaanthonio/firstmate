...
# adapter answers from its own lifecycle record; Grok and Droid answer from
# their isolated rendered-tail fallbacks; a herdr crew's native `busy` is accepted
...
crew_busy_verdict() {  # <target>
  local tail40=''
  case "$HARNESS" in
    grok*|droid*) tail40=$(fm_backend_capture "$TASK_BACKEND" "$1" 40 "$EXPECTED_LABEL" 2>/dev/null) || tail40='' ;;
  esac
...
if [ "$HAVE_RUN" = 1 ]; then
  ...
  if [ "$RUN_SOURCE" = coarse ]; then
    ...
    if [ -n "$outcome" ]; then
      case "$outcome" in
        passed)        RUN_STATE="done"; RUN_DETAIL=$(passed_pr_detail); RUN_EVENT=$(run_event_identity "outcome:$outcome" || true) ;;
        ...
        failed)
          if nm_reclassify_failed_run_as_held_green; then :; else
            RUN_STATE=failed; RUN_DETAIL="run failed"; RUN_EVENT=$(run_event_identity "outcome:$outcome" || true)
          fi ;;
      ...
      case "$status" in
        ...
        failed)
          if nm_reclassify_failed_run_as_held_green; then :; else
            RUN_STATE=failed; RUN_DETAIL="run failed"; RUN_EVENT=$(run_event_identity "status:$status" || true)
          fi ;;
