...
if [ -n "$NEW_EFFORT" ] && [ "$NEW_EFFORT" != default ] &&
  [ "$NEW_EFFORT" != ultra ] && ! fm_profile_effort_valid "$NEW_EFFORT"; then
  die "--effort must be one of default, $FM_PROFILE_EFFORT_VALUES, ultra"
fi
...
resolve_relaunch_profile() {
  ...
  if [ "$KIND" = secondmate ]; then
    ...
    if [ -n "$CONFIG_EFFORT" ] && [ "$CONFIG_EFFORT" != ultra ] &&
      ! fm_profile_effort_valid "$CONFIG_EFFORT"; then
      echo "warning: config/secondmate-harness effort token '$CONFIG_EFFORT' is not one of $FM_PROFILE_EFFORT_VALUES, ultra; ignoring" >&2
      CONFIG_EFFORT=
    fi
  fi
