...
#   endpoint-gone, herdr-native, grok-regex, droid-regex, muse-session-log,
...
# The Grok and Droid arms are the ONLY rendered-text classifications that
# survive the redesign, because neither harness has a verified semantic source.
# Each is scoped to its exact harness and can never classify another adapter.
# The delivery guards in bin/fm-composer-lib.sh match rendered
...
# fm_busy_droid_tail_busy: the Droid-only rendered-tail fallback preserving
# the empirically verified `Press ESC to stop` working footer.
...
# the Grok or Droid arm; when absent that arm captures through fm_backend_capture
