...
#   fm-captain-hold.sh preserve-question <origin-id> <decision-key> \
#     --state existing|unseeded --question-file <path>
#   fm-captain-hold.sh open-questions [--render]
...
#
# These commands preserve the exact question evidence on the captain call that
# already owns it. They do not create another decision ledger or another close
# path: status decisions still close through their keyed status transition, and
# captain-held tasks still close through `answer` or `answers`.
QUESTION_MARKER='droid-afk-question'
QUESTION_FILE_PREFIX='droid-afk-question-'
...
question_evidence_path() {  # <origin> <digest> <reference>
  local origin=$1 digest=$2 reference=$3 directory path
  [ "${#digest}" -eq 64 ] || return 1
  case "$digest" in *[!0-9a-f]*) return 1 ;; esac
  [ "$reference" = "$QUESTION_FILE_PREFIX$digest" ] || return 1
...
park_question() {  # <origin> <digest> <question-text>
  local origin=$1 digest=$2 question=$3 directory reference path staged
  directory="$DATA/$origin"
  reference="$QUESTION_FILE_PREFIX$digest"
  (umask 077; mkdir -p "$directory") \
    || fail "could not create deferred-question evidence directory for $origin"
  [ -d "$directory" ] && [ ! -L "$directory" ] \
    || fail "deferred-question evidence directory is unsafe for $origin"
  path="$directory/$reference"
  if [ -e "$path" ] || [ -L "$path" ]; then
    question_evidence_path "$origin" "$digest" "$reference" >/dev/null \
      || fail "deferred-question evidence is unsafe or corrupted for $origin/$digest"
    [ "$(cat "$path")" = "$question" ] \
      || fail "deferred-question evidence conflicts for $origin/$digest"
    printf '%s' "$reference"
    return 0
  fi
  staged=$(umask 077; mktemp "$directory/.${reference}.XXXXXX") \
    || fail "could not stage deferred-question evidence for $origin/$digest"
  if ! printf '%s' "$question" > "$staged" || ! chmod 0600 "$staged"; then
    rm -f -- "$staged"
    fail "could not stage deferred-question evidence for $origin/$digest"
  fi
  if ! ln "$staged" "$path" 2>/dev/null; then
    rm -f -- "$staged"
    question_evidence_path "$origin" "$digest" "$reference" >/dev/null \
      || fail "could not publish deferred-question evidence for $origin/$digest"
    [ "$(cat "$path")" = "$question" ] \
      || fail "deferred-question evidence conflicts for $origin/$digest"
  else
    rm -f -- "$staged"
...
activate_question_evidence() {  # <origin> <digest> <question-text>
  local reference staged
  reference=$(park_question "$1" "$2" "$3")
  if [ -e "$DROID_DEFERRAL_MARKER" ] || [ -L "$DROID_DEFERRAL_MARKER" ]; then
    [ -f "$DROID_DEFERRAL_MARKER" ] && [ ! -L "$DROID_DEFERRAL_MARKER" ] \
      || fail "Droid deferred-question marker is unsafe"
  else
    staged=$(umask 077; mktemp "$STATE/.droid-afk-question-deferral.XXXXXX") \
      || fail "could not stage Droid deferred-question state"
    : > "$staged"
    chmod 0600 "$staged"
    if ! ln "$staged" "$DROID_DEFERRAL_MARKER" 2>/dev/null; then
      rm -f -- "$staged"
      [ -f "$DROID_DEFERRAL_MARKER" ] && [ ! -L "$DROID_DEFERRAL_MARKER" ] \
        || fail "could not mark Droid deferred-question state"
    else
      rm -f -- "$staged"
    fi
  fi
...
# Output `<origin><TAB><key><TAB><digest><TAB><reference>` for every evidence
# marker in one captain-held task body. The three-field legacy marker remains
# readable for an in-flight pre-collapse row.
hold_question_evidence() {  # <hold-id> <decoded-hold-body>
  local id=$1 body=$2 line marker origin key digest reference extra inferred_origin
  local complete=1
  inferred_origin=$(printf '%s\n' "$body" | sed -n 's/^Origin: //p' | head -1)
  while IFS= read -r line || [ -n "$line" ]; do
    case "$line" in "$QUESTION_MARKER "*) ;; *) continue ;; esac
    IFS=' ' read -r marker origin key digest reference extra <<EOF
$line
EOF
    if [ -z "${reference:-}" ]; then
      reference=$key
      ...
      case "$id" in
        "$origin-decision-"?*) key=${id#"$origin-decision-"} ;;
        *) key=$id ;;
      esac
    fi
    if [ "$marker" != "$QUESTION_MARKER" ] || [ -z "$origin" ] \
        || [ -z "$key" ] || [ -z "$digest" ] || [ -z "$reference" ] \
        || [ -n "${extra:-}" ]; then
      complete=0
      continue
    fi
    printf '%s\t%s\t%s\t%s\n' "$origin" "$key" "$digest" "$reference"
  done <<EOF
$body
EOF
  [ "$complete" -eq 1 ]
...
status_question_evidence() {  # <status-file> <decision-key>
  local status_file=$1 wanted=$2 line key evidence digest reference complete=1
  [ -f "$status_file" ] && [ ! -L "$status_file" ] && [ -r "$status_file" ] || return 1
  while IFS= read -r line || [ -n "$line" ]; do
    key=$(_fm_decision_key "$line") || continue
    [ "$key" = "$wanted" ] || continue
    case "$line" in
      *" | $QUESTION_MARKER "*) evidence=${line##*" | $QUESTION_MARKER "} ;;
      *) continue ;;
    esac
    digest=${evidence%% *}
    reference=${evidence#* }
    if [ -z "$digest" ] || [ "$reference" = "$evidence" ] || [ -z "$reference" ] \
...
hold_is_active() {  # <hold-id>
  ...
  [ "$(show_field_value "$show" held)" = yes ] || return 1
  [ "$(show_field_value "$show" hold_kind)" = captain ] || return 1
}

question_hold_id() {  # <origin> <key>
  local origin=$1 key=$2 legacy migrated rc=0
  if hold_is_active "$key"; then
    printf '%s' "$key"
    return 0
  fi
  legacy=$(legacy_hold_id "$origin" "$key")
  if hold_is_active "$legacy"; then
    printf '%s' "$legacy"
    return 0
  fi
  migrated=$(resolve_migrated_entry "$origin" "$key") || rc=$?
  [ "$rc" -eq 0 ] || return "$rc"
  migrated=${migrated%% *}
  hold_is_active "$migrated" || return 1
  printf '%s' "$migrated"
}

preserve_in_hold() {  # <hold-id> <origin> <key> <digest> <reference>
  local id=$1 origin=$2 key=$3 digest=$4 reference=$5 show body evidence
  local existing_origin existing_key existing_digest existing_reference tmp
  task_show_or_fail "$id" "captain-held task $id disappeared while preserving a question"
  body=$(decode_shown_value "$(show_field "$show" body)") \
    || fail "could not decode the existing body for $id"
  evidence=$(hold_question_evidence "$id" "$body") \
    || fail "captain-held task $id has malformed deferred-question evidence"
  while IFS=$'\t' read -r existing_origin existing_key existing_digest existing_reference; do
    [ "$existing_origin" = "$origin" ] && [ "$existing_key" = "$key" ] \
      && [ "$existing_digest" = "$digest" ] || continue
    [ "$existing_reference" = "$reference" ] \
      || fail "captain-held task $id has conflicting deferred-question evidence"
    return 0
  done <<EOF
$evidence
EOF
  [ -z "$body" ] || body="${body}"$'\n'
  body="${body}${QUESTION_MARKER} ${origin} ${key} ${digest} ${reference}"
  tmp=$(umask 077; mktemp "${TMPDIR:-/tmp}/fm-captain-question.XXXXXX") \
    || fail "could not stage deferred-question evidence for $id"
  if ! printf '%s\n' "$body" > "$tmp" \
...
preserve_in_status() {  # <status-file> <key> <verb> <summary> <digest> <reference>
  local status_file=$1 key=$2 verb=$3 summary=$4 digest=$5 reference=$6 evidence rc=0
  evidence=$(status_question_evidence "$status_file" "$key") \
    || fail "cannot read existing deferred-question evidence for [key=$key]"
  if printf '%s\n' "$evidence" \
  ...
  fm_wake_status_append_self_announced "$STATE" "$status_file" \
    "$verb [key=$key]: $summary | $QUESTION_MARKER $digest $reference" || rc=$?
...
command_preserve_question() {
  ...
  if [ -f "$meta" ]; then
    CAPTAIN_META_LOCK=$(fm_meta_lock_path "$meta") \
      || fail "could not resolve task metadata lock"
    fm_lock_acquire_wait "$CAPTAIN_META_LOCK"
    CAPTAIN_META_LOCK_HELD=1
    [ -f "$meta" ] || fail "origin $origin disappeared while preserving a question"
  fi
  ...
  if [ -n "$id" ]; then
    ...
    preserve_in_hold "$id" "$origin" "$key" "$digest" "$reference"
  ...
  if [ -n "$verb" ]; then
    reference=$(activate_question_evidence "$origin" "$digest" "$question")
    preserve_in_status "$status_file" "$key" "$verb" "$summary" "$digest" "$reference"
    printf 'preserved: status %s/%s\n' "$origin" "$key"
    return 0
  fi

  [ "$state" = unseeded ] || fail \
    "binding claims state=existing but $origin/$key has no active captain-held task and no open status decision; re-attempt with the owner's real key or state=unseeded"

  id=$(legacy_hold_id "$origin" "$key")
  command_hold "$id" --origin "$origin" \
...
encode_question_reference() {  # <origin> <digest> <reference>
  local path
  path=$(question_evidence_path "$1" "$2" "$3") || return 1
  LC_ALL=C base64 < "$path" | LC_ALL=C tr -d '\n'
}
...
emit_question_records() {
  local held_ids='' held_rows='' id show body title hold_evidence
  local origin key digest reference encoded status_file open summary pair
  local item=0 variant complete=1 seen=$'\n'
  ...
  if fm_tasks_axi_compatible 2>/dev/null; then
    if held_rows=$(tasks_axi list --state held --kind captain 2>/dev/null); then
      ...
      printf 'limitation\tcaptain-held tasks could not be listed\n'
      complete=0
    fi
  else
    printf 'limitation\tcompatible tasks-axi is unavailable; captain-held tasks were not read\n'
    complete=0
  fi

  for id in $held_ids; do
    if ! task_show "$id"; then
      printf 'limitation\tcaptain-held task %s could not be read\n' "$id"
      complete=0
      continue
    fi
    show=$TASK_SHOW_OUTPUT
    body=$(decode_shown_value "$(show_field "$show" body)") || {
      printf 'limitation\tcaptain-held task %s has an unreadable body\n' "$id"
      complete=0
      continue
    }
    title=$(show_field_value "$show" title)
    hold_evidence=''
    if ! hold_evidence=$(hold_question_evidence "$id" "$body"); then
      printf 'limitation\tcaptain-held task %s has malformed deferred-question evidence\n' "$id"
      complete=0
    fi
    while IFS=$'\t' read -r origin key digest reference; do
      [ -n "$origin" ] && [ -n "$key" ] && [ -n "$digest" ] && [ -n "$reference" ] || continue
      pair="${origin}/${key}"
      case "$seen" in *$'\n'"$pair"$'\n'*) ;; *)
        item=$((item + 1))
        seen="${seen}${pair}"$'\n'
        printf 'decision\t%s\t%s\t%s\thold\t%s\n' "$item" "$origin" "$key" "$title"
        ;;
      esac
      variant=$(printf '%s\n' "$hold_evidence" \
        | LC_ALL=C awk -F '\t' -v o="$origin" -v k="$key" -v d="$digest" '
            $1 == o && $2 == k {
              n++
              if ($3 == d) { print n; exit }
            }
          ')
      if ! encoded=$(encode_question_reference "$origin" "$digest" "$reference"); then
        printf 'limitation\tcaptain-held task %s has unreadable deferred-question evidence %s\n' "$id" "$reference"
        complete=0
        continue
      fi
      emit_question_chunks "$item" "$variant" "$encoded"
...
EOF
  ...
  for status_file in "$STATE"/*.status; do
    ...
    if [ ! -f "$status_file" ] || [ -L "$status_file" ] || [ ! -r "$status_file" ] \
        || ! sed -n '' "$status_file" >/dev/null 2>&1; then
      printf 'limitation\tstatus owner %s could not be read safely\n' "$origin"
      complete=0
      continue
    fi
    ...
    while IFS=$'\t' read -r key _verb summary; do
      ...
      variant=0
      hold_evidence=''
      if ! hold_evidence=$(status_question_evidence "$status_file" "$key"); then
        printf 'limitation\tstatus owner %s/%s has malformed deferred-question evidence\n' "$origin" "$key"
        complete=0
      fi
      while IFS=$'\t' read -r digest reference; do
        [ -n "$digest" ] && [ -n "$reference" ] || continue
        if ! encoded=$(encode_question_reference "$origin" "$digest" "$reference"); then
          printf 'limitation\tstatus owner %s/%s has unreadable deferred-question evidence %s\n' \
            "$origin" "$key" "$reference"
          complete=0
          continue
        fi
        variant=$((variant + 1))
...
command_open_questions() {
  ...
  records=$(mktemp "${TMPDIR:-/tmp}/fm-open-questions.XXXXXX") \
    || fail "could not stage the outstanding-question listing"
...
command_answers() {
  ...
  while IFS= read -r row; do
    ...
    valid=1
    case "$row" in
      *"$tab"*) key=${row%%"$tab"*}; rest=${row#*"$tab"} ;;
      *) key=''; rest=''; valid=0 ;;
    esac
    case "$rest" in
      *"$tab"*) answer=${rest%%"$tab"*}; rest=${rest#*"$tab"} ;;
      *) answer=''; rest=''; valid=0 ;;
    esac
    label=${rest%%"$tab"*}
    case "$rest" in
    ...
    case "$mode" in *"$tab"*) valid=0 ;; esac
    case "$key" in ''|*[!A-Za-z0-9._-]*) valid=0 ;; esac
    [ "${#key}" -le 128 ] || valid=0
    answer=$(sanitize_field "${answer:-}")
    [ -n "$answer" ] || valid=0
    label=$(sanitize_field "${label:-}")
    if [ "$valid" -ne 1 ]; then
      printf 'skipped: invalid keyed-answer record (expected task id, answer, label, and optional close mode TSV fields)\n'
      skipped=$((skipped + 1))
      continue
    fi
  ...
  if [ "$has_meta" = 1 ]; then
    ...
    if [ -n "$keys" ]; then
      while IFS=$'\t' read -r key _verb _summary; do
        [ -n "$key" ] || continue
        question_evidence=''
        question_evidence=$(status_question_evidence "$status_file" "$key") \
          || fail "cannot read deferred-question evidence for $origin/$key"
        if [ -n "$question_evidence" ]; then
          owner=''
          while IFS= read -r owner_entry; do
            [ -n "$owner_entry" ] || continue
            resolved=$(resolve_entry "$origin" "$owner_entry") || exit $?
            resolved=${resolved%% *}
            if [ "$owner_entry" = "$key" ] || [ "$resolved" = "$key" ] \
                || [ "$resolved" = "$(legacy_hold_id "$origin" "$key")" ]; then
              owner=$resolved
              break
            fi
          done <<EOF
...
EOF
          ...
          while IFS=$'\t' read -r question_digest question_reference; do
            ...
            question_evidence_path "$origin" "$question_digest" "$question_reference" >/dev/null \
              || fail "cannot transfer unreadable deferred-question evidence for $origin/$key"
            preserve_in_hold "$owner" "$origin" "$key" "$question_digest" "$question_reference"
          done <<EOF
$question_evidence
EOF
          release_task_control_lock
