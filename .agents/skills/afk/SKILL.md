...
- Droid's tracked `AskUser` PreToolUse guard denies an interactive question without answering it while `state/.afk` exists.
  Native `AskUser` carries one freeform questionnaire and no owner field, so every question block Firstmate attempts while away must put this binding on its own line immediately after the `[question]` line:
  ...
  `state=existing` requires the named key to resolve to an active captain-held task or an open status decision.
  `state=unseeded` authorizes the captain-call owner to create an ordinary captain-held task for that origin and key.
  `key=derive` derives a stable key from the origin, question, and ordered options.
  A topic is descriptive UI text, never an owner.
  Missing, malformed, or stale bindings are rejected rather than inferred.
  `bin/fm-captain-hold.sh` owns exact question preservation and the outstanding-question projection without adding a parallel decision ledger or close path.
  Continue every independently authorized action instead of retrying `AskUser` or asking the question in plain text.
  Ordered return catch-up presents every outstanding deferred question in full, and presentation never counts as approval or resolution.
...
- The active backend passes its capture plus declarative styled, cursor, identity, and row capabilities to the shared screen classifier; all structural recognition and verdict logic remains in `bin/fm-composer-lib.sh`.
  ...
  `FM_BUSY_REGEX` overrides the rendered delivery guards plus the isolated Grok and Droid task-state fallbacks.
