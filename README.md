...
- **Two task shapes** - ship tasks deliver authorized changes with review-ready descriptions and UI evidence; scout tasks leave standalone investigation reports when the intake contract warrants separate research.
...
- A verified primary agent harness: Claude Code, Grok, Pi, `pi-signed`, Oh My Pi (`omp`), Codex, OpenCode, Cursor Agent CLI, or Droid.
...
Droid is verified as a primary too, using tracked project-scope `.factory/settings.json` hooks and bounded foreground checkpoints.
A normal interactive `droid` invocation loads that file without a launch flag.
...
Claude, grok, and droid use the slash form shown here; codex uses the same names with `$`, such as `$afk`.
...
- [docs/supervision-protocols/](docs/supervision-protocols/) - rendered primary-harness watcher protocols for Claude, Codex, OpenCode, Pi and `pi-signed`, omp, Grok, Cursor, Droid, and unknown harness fallback.
